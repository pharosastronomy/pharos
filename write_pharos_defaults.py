...
def write_pharos_defaults( pharos, pathdefs, pathlog, pathans, pathdata, pathbias, pathdark, pathflat, stringbias, stringdark, stringflat, namebias, namedark, nameflat, psfs, stars, cprefix, sradius, aradius, cradius, starnumber, skyskew, skyfit, gain, linlevel, satslevel, verbose, tsleep, fsleep, tfile, cfile, port, obsid, band, sslep, dpath, tdfile, public, cdfile, tfits, cfits, lport, lfile, lband, ltfits, lcfits ):
    ...
    defs_file.write("param pharosdef\n")
    defs_file.write(" pharos  entry {pharos location}       \""        +   pharos + "\"\n" )
    defs_file.write(" pathdefs    entry {pharos settings} \""              +   pathdefs   + "\"\n" )
    defs_file.write(" pathlog     entry {pharos logfile}               \"" +   pathlog    + "\"\n" )
    ...
    defs_file.write("pharos Settings\n")
    defs_file.write("*\n")
    defs_file.write("menu\n")
    defs_file.write("$param(pharosdef);  $param(gifparams); $param(sourceparams); xterm -hold -sb -sl 2000 -e bash -c \"$pharos/write_pharos_defaults.py $pharos $pathdefs $pathlog $pathans $pathdata $pathbias $pathdark $pathflat $stringbias $stringdark $stringflat $namebias $namedark $nameflat $stars $psfs $cprefix $sradius $aradius $cradius $starnumber $skyskew $skyfit $gain $linlevel $satslevel $verbose $tsleep $fsleep $tfile $cfile $port $obsid $band $sslep $dpath $tdfile $public $cdfile $tfits $cfits $lport $lfile $lband $ltfits $lcfits \" \n")
    defs_file.write("\n")

    defs_file.write("RUN pharos data reduction\n")
    defs_file.write("*\n")
    defs_file.write("menu\n")
    defs_file.write("$param(pharosdef); xterm -hold -sb -sl 2000 -e \"tcsh -c \\\"python -u $pharos/run_pharos.py $xpa_method $pathdefs |& tee $pathlog\\\" \" \n")
    defs_file.write("\n")

    defs_file.write("pharos Fake Data Module\n")
    defs_file.write("*\n")
    defs_file.write("menu\n")
    defs_file.write("$param(pharosdef); $param(fakedata); xterm -hold -sb -sl 2000 -e tcsh -c \"$pharos/pharos_fakedata.py $fsleep $tfile $cfile \" \n")
    defs_file.write("\n")

    defs_file.write("pharos Server Module\n")
    defs_file.write("*\n")
    defs_file.write("menu\n")
    defs_file.write("$param(pharosdef); $param(server); xterm -hold -sb -sl 2000 -e tcsh -c \" $pharos/pharos_server_module.py $port $obsid $band $tdfile $tsleep -public $public --path $dpath --compfile $cdfile --thumbtarget $tfits --thumbcomp $cfits \" \n")
    defs_file.write("\n")

    defs_file.write("pharos Client Logger Module\n")
    defs_file.write("*\n")
    defs_file.write("menu\n")
    defs_file.write(" $param(pharosdef); $param(logger); xterm -hold -sb -sl 2000 -e tcsh -c \" $pharos/pharos_client_logger.py $lport -filter $lband -logfile $lfile -thumbtarget $ltfits -thumbcomp $lcfits \" \n")
    defs_file.write("\n")
...
if  __name__ == "__main__":
    ...
    write_pharos_defaults( pharos, pathdefs, pathlog, pathans, pathdata, pathbias, pathdark, pathflat, stringbias, stringdark, stringflat, namebias, namedark, nameflat, psfs, stars, cprefix, sradius, aradius, cradius, starnumber, skyskew, skyfit, gain, linlevel, satslevel, verbose, tsleep, fsleep, tfile, cfile, port, obsid, band, sslep, dpath, tdfile, public, cdfile, tfits, cfits, lport, lfile, lband, ltfits, lcfits)
