...
run_pharos     - Initiates the enviroment variables and starts the sequence
main           - Main program start

Runs with: run_pharos.py <xpa handle> pharos.defaults
...
#                str(aperatlist[i][1])+" "+str(seeing)+" "+filename+" \n")
    ...
    ### new round of pharos of the same data. Perhaps we should make pharos
...
def seekfits(rtdefs, dataref, dirs, tsleep, comparisons, targets, psf_fwhm):
                ...
                if (filename.endswith('.fits') or filename.endswith('.fit') or filename.endswith('.FIT'):
                    ...
                    if rtdefs['liveplot']:
                       ...
                       if count==1:
                          ...
                          with open("pharos_liveplot.csh", "w") as outfile:
                          ...
                          subprocess.call("source ./pharos_liveplot.csh", shell=True)
    ...
    print
    print "==== pharos END ==== " + time.strftime('%X %x %Z') 
    print
    print         
...
#############################################################################
def run_pharos(pharosdir, xpapoint, pathdefs):
# requires get_comps_fwhm, seekfits
  
    print
    print
    print
    print "==== pharos START ==== " + time.strftime('%X %x %Z')
    print
    ...
    call(['ln', '-s', pharosdir+'/Timing/jpleph.dat', 'JPLEPH'])
    call(['ln', '-s', pharosdir+'/Timing/leap.dat', 'leapdat'])
    os.chdir(data_dir) # Move back to the data directory
    print
    print 'ln', '-s', pharosdir+'/Timing/jpleph.dat', 'JPLEPH'
    print
    ...
    if rtdefs['liveplot']:
       ...
       subprocess.call('rm -rf *_tmp *_diff liveplot.py comp.fits target.fits \
                       liveplotdata.txt pharos_liveplot.csh', shell=True) 
       os.chdir(prev_dir)   
...
if  __name__ == "__main__":
    ...
    import sys
    pharosdir, dummy = os.path.split(sys.argv[0])
    xpapoint       = sys.argv[1]
    pathdefs       = sys.argv[2]
    run_pharos(pharosdir, xpapoint, pathdefs)
