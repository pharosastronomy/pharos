#!/usr/bin/env python
# pharos client module which receives data from server module
# and logs into a flat ascii file.

# Usage:
# pharos_client_logger.py [port] 
...
# Examples:
# pharos_client_logger.py tcp://localhost:5556 
# pharos_client_logger.py tcp://92.96.59.99:5556 -logfile mylog.dat
...
parser = argparse.ArgumentParser(description='pharos client logger')
...
print "pharos: Collecting updates from broadcasting server %s" % args.port
...
if args.logfile is not None: 
    ...
    outfile = "pharos_log_UTC"+ now.replace(" ", "_")+".txt"
    print 'Logfile:', outfile
...
myfile.write("#pharos: Log start UTC time %s \n" % now)
myfile.close()
