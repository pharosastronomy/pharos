#!/usr/bin/env python
# pharos live plotting client module which either
# a) receives data from server module live, in which case nothing is saved 
# or
# b) monitors and plots the updating of logged data created by pharos_client_logger.py

# Usage:
# pharos_client_liveplot.py {-port [port] | -logfile [filename]} -filter bandpass
...
# Examples:
# pharos_client_liveplot.py -port tcp://localhost:5556 
# pharos_client_liveplot.py -logfile mylogfile.dat 
...
def init_datapoins(tempfile, now, bjdswitch):
    ...
    print  "pharos live plot: initial read found %s rows of data in log" % str(nline)
...
def append_datapoints(columns, xdata, yrawtarget, yrawtargeterr, \
    ...
    print "pharos: datapoint at time %s added" % str(UTCdatetime)
...
parser = argparse.ArgumentParser(description='pharos client live plotter')
...
if args.logfile is not None:
    uselog = True
    if port is not None:
        print "!pharos: you have specified both port and logfile to monitor!"
        print "This is not possible. Continuing with using the logfile."
else:
    uselog = False
    print "pharos: no log file set. Plotting live data only."
    if port is None: 
        print "Set -port or -logfile!"

# bandpass filter must be at the beginning of the messages sent
if args.filter is not None:
    print "pharos: filtering messages, bandpass %s" % args.filter
...
######### while
        while True: 
            ...
            if datafiletime != lastdatafiletime:
                ...
                print now + " pharos logfile was updated."
                ...
                for line in fp:
                    ...
                    if columns[0] == '#pharos:':
                        ...
                        fig.canvas.show()
                        print "pharos: currently displayed %s points" % str(npoints_current)
############# end if datafiletime !=
            else: 
...
###############################
     ...
     print "pharos: Collecting updates from broadcasting server ", port
     ...
     try: 
        while True:
            ...
            print "pharos: message received %s" % now
